"""Per-action eval for v10c (VeraRetouch-style per-pixel MLP renderer).

v10c REPLACES the Bezier+CN+attention pipeline with a fully data-driven
per-pixel MLP conditioned on global latent z = MLP(visual + action_emb).
Inspired by [R3] VeraRetouch (arxiv 2604.27375).
"""
...
def main():
    ...
    ckpt_path = 'checkpoints/lut_v10c_vera/best.pt'
    ...
    gate_val = float(model.vera.gate.item()) if model.use_vera_renderer else 0.0

    print(f'\nv10c_vera best_val_psnr={ckpt.get("val_psnr",0):.2f}dB '
          f'@ Ep{ckpt.get("epoch","?")}')
    print(f'VeraRenderer gate (residual magnitude): {gate_val:+.4f}')
    print(f'\nv10c_vera Per-action PSNR:')
