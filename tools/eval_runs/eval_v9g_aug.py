"""Quick per-action eval for v9g_aug checkpoint."""
...
def main():
    ...
    ckpt_path = 'checkpoints/lut_v9g_aug/best.pt'
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    ca = ckpt['args']

    model = NamedCurvesPredictor(
        n_colors=ca['nc_n_colors'],
        n_control_points=ca['nc_n_control_points'],
    ...
    with torch.no_grad():
        for b in loader:
            ...
            a = b['action'][0]
            per_action[a].append(psnr)
            if a == 'wb' and hasattr(model, '_last_wb_log_gains'):
                gains_wb.append(torch.exp(model._last_wb_log_gains[0]).cpu().tolist())

    print(f'\nv9g_aug ckpt: best_val_psnr={ckpt.get("val_psnr",0):.2f}dB @ Ep{ckpt.get("epoch","?")}')
    print(f'\nv9g_aug Per-action PSNR:')
    ...
    if gains_wb:
        arr = np.array(gains_wb)
        print(f'\nv9g_aug WB gains for wb action samples '
              f'(R/G/B avg over {len(gains_wb)}):')
        print(f'  R={arr[:,0].mean():.3f}  G={arr[:,1].mean():.3f}  '
              f'B={arr[:,2].mean():.3f}')
