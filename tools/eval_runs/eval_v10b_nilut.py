"""Per-action eval for v10b (v9a + NILUT residual color transform).

v10b adds a NILUT (small MLP, neural implicit 3D LUT) AFTER the Bezier blend
to learn off-Planckian / hue-rotation / split-toning residuals that 7D ISP
cannot represent. Inspired by [R6] NILUT (AAAI 2024).
"""
...
def main():
    ...
    ap.add_argument('--ckpt', type=str,
                    default='checkpoints/lut_v10b_nilut/best.pt')
    args = ap.parse_args()
    ...
    # NILUT gate value (shows how much residual the model learned to use)
    gate_val = float(model.nilut.gate.item()) if model.use_nilut_residual else 0.0

    print(f'\ncheckpoint: {ckpt_path}')
    print(f'v10b_nilut best_val_psnr={ckpt.get("val_psnr",0):.2f}dB '
          f'@ Ep{ckpt.get("epoch","?")}')
    print(f'NILUT gate (residual magnitude): {gate_val:+.4f}')
    print(f'\nv10b_nilut Per-action PSNR:')
