...
class MobileVenus(nn.Module):
    ...
    def load_venus_weights(
        ...
        """
        从 Venus 预训练权重初始化 MobileVenus
        
        分模块加载策略:
        - 视觉编码器: Venus CLIP-ViT → MobileViT (权重映射)
        - 美学评分器: Venus scorer → MobileVenus scorer (直接/自适应加载)
...
def create_mobile_venus(
    pretrained: bool = False,
    pretrained_path: Optional[str] = None,
    venus_pretrained_path: Optional[str] = None,
    config: Optional[MobileVenusConfig] = None
) -> MobileVenus:
    """
    创建 MobileVenus 模型
    
    Args:
        pretrained: 是否加载 MobileVenus 预训练权重
        pretrained_path: MobileVenus 权重路径
        venus_pretrained_path: Venus 权重路径（用于迁移学习初始化）
        config: 模型配置
        
    Returns:
        model: MobileVenus 模型
    """
    if config is None:
        config = MobileVenusConfig()
    
    model = MobileVenus(config)
    
    # 加载 MobileVenus 自己的权重
    if pretrained and pretrained_path is not None:
        logger.info(f"Loading MobileVenus weights from {pretrained_path}")
...
if __name__ == "__main__":
    ...
    model = create_mobile_venus(config=config)
