...
基于 v7 架构, 核心改进:
  1. FiveK (Expert A/C/D/E) + PPR10K (Expert A/B/C) 联合训练
  2. 保留 v7 的退化-修复对比学习
...
def main():
    ...
    logger.info(f'对比学习: {"ON" if use_contrastive else "OFF"}  '
                f'PPR10K: {"ON" if use_ppr10k else "OFF"}')
    ...
    # v8.1: FiveK 3x weighted sampling
    from torch.utils.data import WeightedRandomSampler
    n_fk = len(fivek_train)
    n_pk = len(ppr10k_train)
    sw = [3.0]*n_fk + [1.0]*n_pk
    sampler = WeightedRandomSampler(sw, num_samples=len(sw), replacement=True)
    logger.info("v8.1 sampler: FiveK=%d x3, PPR10K=%d x1" % (n_fk, n_pk))
