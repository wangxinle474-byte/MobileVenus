...
def _make_denorm_fn():
    ...
    def denorm(norm_params, device):
        lo = torch.tensor(lo_list, device=device).float()
        hi = torch.tensor(hi_list, device=device).float()
        if norm_params.dim() == 1:
            return norm_params * (hi - lo) + lo
        return norm_params * (hi - lo).unsqueeze(0) + lo.unsqueeze(0)
